...
## 座標系
### ディスプレイ座標系
`SurfaceView`のデフォルト座標原点はディスプレイ左下で上方向が正、単位はピクセルです。しかしブロックを下に落とす描画をすると座標計算に手間がかかるので、描画メソッド(`MainSurfaceView`クラスの内部クラス`DrawCanvas.draw()`)で以下のように変換しています。
- 原点はディスプレイ左上
- ディスプレイ下方向が正  
つまり、座標はx成分y成分ともに正の数となります。
### ワールド座標系
JBox2Dでのワールド座標の原点は左下で上方向が正、重力は負方向、単位はメートルです。
### ワールド座標系からディスプレイ座標系への変換
TODO:そのうち書く。
## タイル描画
